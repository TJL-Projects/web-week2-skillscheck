...
let lovesCodeChecker = x => x === true ? true : false
...
let oddChecker = num => num % 2 !== 0 ? 'the number is odd' : 'the number is even'
...
let bigOrSmall = arr => {
	let answers = arr.map( element => {
		return element > 100 ? 'big' : 'small'
...
let filtered = myNumbers.filter(element => element > 100 ? element: null)
...
let total = myNumbers.reduce( (acc, cr) => {
	return acc += cr
}, 0)
...
let enemies = people.filter(e => e.friend === false ? e: null)
